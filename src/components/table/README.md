...
Обратите внимание, что по умолчанию нижний колонтитул таблицы не отображается, если для `foot-clone`
...
Свойство `sort-compare-locale` по умолчанию имеет значение `undefined`, что означает использование
локали браузера (или среды выполнения Node.js) по умолчанию. Свойство `sort-compare-locale` может
принимать либо строку с языковым тегом [BCP 47](https://www.rfc-editor.org/info/bcp47), либо
_массив_ таких тегов. Для получения дополнительной информации о локалях смотрите раздел
[Идентификация и согласование локали](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl#Locale_identification_and_negotiation)
на MDN.
