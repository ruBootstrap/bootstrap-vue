...
- Окна сообщений являются расширением компонента `<b-modal>` и, следовательно, поддерживают большинство свойств `<b-modal>` (в формате <samp>camelCase</samp>), за исключением следующих свойств: `lazy`, `static`, `busy`, `visible`, `noStacking`, `okOnly`, `okDisabled` и `cancelDisabled`.
